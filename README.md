...
#### Android Versions up to 7.x (Nougat)
  ...
  2. **Download the app.**
     ...
     https://github.com/Geekgasm/kintrol/releases/download/0.27/kintrol-v0.27.apk
     to download the latest version 0.27 of the Kintrol app.
...
#### Android Versions 8.x (Oreo) and higher

Allowing apps to be installed from sources other than the Google Play
store has changed with Oreo. Instead of allowing (or disallowing) any
and all unknown apps to be installed, you now allow which apps
(sources) are allowed to install other apps.

1. **Download the app.**
   In the web browser of your android device, follow the link
   https://github.com/Geekgasm/kintrol/releases/download/0.27/kintrol-v0.27.apk
   to download the latest version 0.27 of the Kintrol app.
   There might be a warning about downloads from unknown sources.
   Select that you want to keep the downloaded APK file.

2. **Open the downloaded file.**
   Pull down the status bar at the top of the screen to show the
   notifications. You should see one saying that the download for the
   Kintrol APK is finished. Tap on it.

3. **Allow the installation**
   If you haven't allowed it before, there will be a popup telling you
   that for security reasons installation from the app by Chrome is
   prevented. Select to go to the settings.

4. **Trust Chrome**
   In the next dialog tap on the slider to trust Chrome. Then tap on the
   back button.

4. **Install the app.**
   Now the installation screen for Kintrol should be visible. Tap on
   Install to install the app.

5. **Revoke the installation privileges from Chrome**
   It's a bad idea to let Chrome install just anything from now on,
   so you should revoke these privileges again right after Kintrol is
   installed. To do that:

   1. Go to the Android *Settings*
   2. Tap on *Apps and Notifications*
   3. Go to *Special Access* at the bottom (you might need to expand
     the *Extended* entry)
   4. Tap on the *Install other apps* permission
   5. Tap on *Chrome*
   6. Switch off the *Trust app from this source* switch.
...
The display will show one of these three values:
  ...
  - *No WIFI* if your Android device is not connected to any WiFi
    network. In this case, enable the WiFi access to the network to
    which your Linn device is connected.
